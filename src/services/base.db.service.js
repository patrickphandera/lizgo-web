...
class BaseDbService {
  ...
  async deleteByFilter(filter) {
    try {
      ...
      console.error(
        `Failed to delete records by filter from ${this.endpoint}:`,
        error
      );
      throw error;
    }
  ...
  async bulkAction(action, ids, data = {}) {
    try {
      ...
      console.error(
        `Failed to perform bulk action "${action}" on ${this.endpoint}:`,
        error
      );
      throw error;
    }
    ...
    Object.entries(params).forEach(([key, value]) => {
      ...
      if (key === 'filter' && typeof value === 'object') {
        Object.entries(value).forEach(([filterKey, filterValue]) => {
          parts.push(`${encodeURIComponent(`filter.${filterKey}`)}=${encodeURIComponent(filterValue)}`);
        });
